...
import { parseCurrency, showToast, fmtNum, fmtDate, getProgressColor } from '@/lib/utils';

export function ObraModal({ isOpen, onClose, editingId }: { isOpen: boolean; onClose: () => void; editingId: string | null }) {
  const { obras, mapps, medicoes, configs, currentYear, setSyncState } = useData();
  ...
  const handleCurrency = (str: string, setter: (v: string) => void) => {
  ...
  const addMedicao = () => {
    ...
    const mTotalCheck = medsList.reduce((s: number, m: any) => s + (Number(m.valor) || 0), 0);
    if (valorObra > 0 && (mTotalCheck + v) > valorObra) {
  ...
  const removeMedicao = (id: number) => {
    setMedsList(medsList.filter((m: any) => m._tmpId !== id));
  };
  ...
  const saveMedicoesDireto = async () => {
    ...
    await db.from('medicoes').delete().eq('obra_id', editingId);
    if (medsList.length > 0) {
      const toInsert = medsList.map((m: any) => ({
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={editingId ? 'Editar Obra' : 'Nova Obra'} footer={currentFooter} maxWidth="1000px">
      <div className="modal-tabs">
        <div className={`modal-tab ${tab === 'dados' ? 'active' : ''}`} onClick={() => setTab('dados')}>📋 Dados</div>
      ...
      <div className={`modal-tab-content ${tab === 'dados' ? 'active' : ''}`}>
        <div className="form-grid obra-grid">
          <div className="field full">
            <label>Descrição da Obra *</label>
            <textarea value={formData.descricao} onChange={e => setFormData({ ...formData, descricao: e.target.value })} placeholder="Descreva a obra..."></textarea>
          </div>
          <div className="field">
            <label>Empresa *</label>
            <select value={formData.empresa} onChange={e => setFormData({ ...formData, empresa: e.target.value })}>
              <option value="">Selecione</option>
              {cfgEmpresas.map(c => <option key={c.valor} value={c.valor}>{c.valor}</option>)}
            </select>
          </div>
          <div className="field">
            <label>Unidade *</label>
            <select value={formData.unidade} onChange={e => setFormData({ ...formData, unidade: e.target.value })}>
              <option value="">Selecione</option>
              {cfgUnidades.map(c => <option key={c.valor} value={c.valor}>{c.valor}</option>)}
            </select>
          </div>
          <div className="field">
            <label>Cidade</label>
            <select value={formData.local} onChange={e => setFormData({ ...formData, local: e.target.value })}>
              <option value="">Selecione</option>
              {cfgCidades.map(c => <option key={c.valor} value={c.valor}>{c.valor}</option>)}
            </select>
          </div>
          <div className="field">
            <label>Região</label>
            <select value={formData.regiao} onChange={e => setFormData({ ...formData, regiao: e.target.value })}>
              <option value="">Selecione</option>
              {cfgRegioes.map(c => <option key={c.valor} value={c.valor}>{c.valor}</option>)}
            </select>
          </div>
          <div className="field">
            <label>Nº do Processo (NUP)</label>
            <input value={formData.nup} onChange={e => setFormData({ ...formData, nup: e.target.value })} placeholder="NUP 47011.XXXXX/XXXX-XX" />
          </div>
          <div className="field">
            <label>Ordem de Serviço</label>
            <input value={formData.os} onChange={e => setFormData({ ...formData, os: e.target.value })} placeholder="Nº da OS" />
          </div>
          <div className="field">
            <label>MAPP *</label>
            <select value={formData.mapp} onChange={e => setFormData({ ...formData, mapp: e.target.value })}>
              <option value="">Selecione o MAPP</option>
              {nums.map(n => <option key={n} value={n}>MAPP {n}</option>)}
            </select>
            ...
            {inlineMappOpen && (
              <div className="mapp-inline-form open">
                ...
                <div style={{ textAlign: 'right', marginTop: '8px' }}>
                  <button className="btn btn-primary" style={{ fontSize: '11px', padding: '5px 10px' }} onClick={handleCreateMapp}>Salvar MAPP</button>
                </div>
              </div>
            )}
          </div>
          ...
          <div className="field full">
            ...
            <textarea value={formData.obs} onChange={e => setFormData({ ...formData, obs: e.target.value })} placeholder="Observações adicionais..."></textarea>
          </div>
        </div>
      </div>
      ...
      <div className={`modal-tab-content ${tab === 'medicoes' ? 'active' : ''}`}>
        <div className="progress-summary" style={{ marginBottom: '14px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '6px' }}>
            <div style={{ flex: 1, background: 'var(--bg)', borderRadius: '4px', height: '10px', overflow: 'hidden' }}>
              <div style={{ width: `${pct}%`, height: '100%', background: pcColor, borderRadius: '4px' }}></div>
            </div>
            <span style={{ fontFamily: "'Merriweather', serif", fontSize: '14px', color: pcColor, fontWeight: 700, minWidth: '40px' }}>{pct}%</span>
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '11px', color: 'var(--muted)', fontFamily: "'Source Code Pro', monospace", flexWrap: 'wrap', gap: '4px' }}>
        ...
        <div className="medicao-form">
          <div className="form-grid">
            ...
            <div className="field full">
              ...
              <select value={newMed.num} onChange={e => setNewMed({ ...newMed, num: e.target.value })}>
                <option value="1ª Medição">1ª Medição</option>
                <option value="2ª Medição">2ª Medição</option>
                <option value="3ª Medição">3ª Medição</option>
                <option value="4ª Medição">4ª Medição</option>
                <option value="5ª Medição">5ª Medição</option>
                <option value="6ª Medição">6ª Medição</option>
          ...
          <div className="field" style={{ marginTop: '10px' }}>
            ...
            <input type="text" value={newMed.desc} onChange={e => setNewMed({ ...newMed, desc: e.target.value })} placeholder="Ex: Serviços..." />
          </div>
          <div style={{ marginTop: '12px', textAlign: 'right' }}>
            <button className="btn btn-primary" onClick={addMedicao}>＋ Adicionar Medição</button>
          </div>
        </div>

        <div id="medicoes-list">
          {medsList.length === 0 ? (
            <div className="empty-med">📏 Nenhuma medição ainda.</div>
          ) : medsList.map((m: any, i: number) => {
            const acc = medsList.slice(0, i + 1).reduce((s: number, mj: any) => s + (Number(mj.valor) || 0), 0);
